...
public record LitigationCostResponse(
    ...
    @Schema(description = "소액사건 여부(소가 3천만원 이하)", example = "false") boolean isSmallClaim,
    @Schema(description = "전자소송 여부(인지액 10% 감경 대상)", example = "true") boolean isElectronicFiling,
    @Schema(description = "인지액(전자소송 감경 반영 후 금액)", example = "45000") long stampFee,
    ...
    @Schema(description = "당사자 1인당 송달 회차(일반 15회, 소액 10회)", example = "15") int deliveryCount,
